...
The following steps were taken before copying the contents of the src/ folder to this repo and committing.
 ...
 4. Edited turbo.makefile to import ontologies without making them a seed. Started with pdro and ncbitaxon as examples of small and large source ontologies. Copied Standard constants and Import sections from Makefile and edited to list "IMPORTS =  pdro ncbitaxon" and adjusted the ONTOLOGY statements for these (e.g. "mirror/pdro.trigger: $(SRC)". Copied contents for pdro and ncbitaxon from term_lists and extracts in ontologies/robot_implementation to terms.txt and import.owl files in imports. Included "IMP_LARGE=false" to prevent large ontologies from being mirrored. Tested this with NCBITaxon (owl file is 1.87 GB). Made "if [ $(MIR) = true ] && [ $(IMP) = false ];" for pdro to see if can get to just use imports as is. 
 5. Updated turbo-edit.owl for direct imports of all owl files in imports.
 6. Ran "make all". This created the files that were copied over and committed on Jan 21, 2022. The pdro_import.owl got updated to contain the full dependencies rather than the minimal owl that was there. Was able to prevent that previously using a particular series of make commands instead of "make all". Will need to reproduce when adding new imports.
